...
This is a graph of CPU/Memory utilization from SAR (System Activity Report).
...
extract_cswch <- function(rawSarData){
    # Context Switch ====
    header_pattern_cpu <- "proc/s.*cswch/s"
    footer_pattern_cpu <- "^Average:"
    # Find the start of the CPU section, drop into a vector ====
    sar_cpu_headers <- grep(header_pattern_cpu, rawSarData)
    top_of_section <- sar_cpu_headers[1]
...
datafile <- "/home/kwan/Downloads/SAR/kll.sar"
sar_raw <- readLines(datafile)

# sar_title <- sar_raw[1] %>% strsplit("\t") %>% unlist()
sar_title <- sar_raw[1] %>% strsplit("[[:blank:]]")
# sar_title <- sar_raw[1] %>% strsplit("\t") %>% unlist()
report_date <- parse_date_time(sar_title[[1]][5], c("ymd", "mdy"))
report_hostname <- parse_date_time(sar_title[[1]][3], c("ymd", "mdy")) %>% gsub("()", "")
...
#     geom_area(fill="darkgreen", alpha=0.7) + 
  ...
  cpu_graphs <- cpu_plot_both(sar_cpu)
